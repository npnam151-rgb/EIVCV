...
const CVInputForm: React.FC<CVInputFormProps> = ({ onProcess }) => {
  ...
  const processFile = (file: File, type: 'cv' | 'jd' | 'photo') => {
    ...
    // Explicitly block Word files and instruct user to copy-paste
  ...
  const handleDrop = (e: React.DragEvent, type: 'cv' | 'jd' | 'photo') => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleDragOver = (e: React.DragEvent, type: 'cv' | 'jd' | 'photo') => {
    e.preventDefault();
    e.stopPropagation();
    if (type === 'photo') setIsDraggingPhoto(true);
  };

  const handleDragLeave = (e: React.DragEvent, type: 'cv' | 'jd' | 'photo') => {
    e.preventDefault();
    e.stopPropagation();
    if (type === 'photo') setIsDraggingPhoto(false);
  };
      ...
      <div className="flex flex-col space-y-3">
        ...
        <div 
          ...
          onDrop={(e) => {
            ...
            if (droppedFile) processFile(droppedFile, type);
          }}
        ...
        <p className="text-[10px] text-slate-400 italic">Lưu ý: Nếu dùng file Word, vui lòng mở file, chọn tất cả văn bản (Ctrl+A) rồi DÁN vào ô trên.</p>
      </div>
    );
  };
